...
class IntCodeComputer():
    ...
    def run_instruction(self, ip, ints):
        ...
        if (opcode == 1): # addition. 3 parameters
            ...
            print(f'Input # {self._input_instruction}')
...
for amp_setting in amp_settings:
    ...
    ampA = IntCodeComputer(amp_setting[0])
    ampB = IntCodeComputer(amp_setting[1])
    ampC = IntCodeComputer(amp_setting[2])
    ampD = IntCodeComputer(amp_setting[3])
    ampE = IntCodeComputer(amp_setting[4])

    ampA_input = 0
    return_value = 0
    while return_value != -1:
        ampA.set_input(ampA_input)
        return_value |= ampA.execute()
        ampB.set_input(ampA.output)
        return_value |= ampB.execute()
        ampC.set_input(ampB.output)
        return_value |= ampC.execute()
        ampD.set_input(ampC.output)
        return_value |= ampD.execute()
        ampE.set_input(ampD.output)
        return_value |= ampE.execute()
        ampA_input = ampE.output
